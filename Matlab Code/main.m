...
dl0 = 20; % km, length of pipeline segment
...
%         p_rp(ij,h) = (2*0.9*sigma_ut*wt(ij)) / (M_fl*D(ij));
        p_rp(ij,h) = (2*0.99*sigma_ut*wt(ij)) / (M_fl*D(ij));
    end
end
...
% Info_components = [ [zeros(1,6),3,3]; Info_components];
...
counter = 0;
for s = 1:nSystemState
    if max(stateProbability(:,s)) < 1e-6 % 在所有年份概率都比较小，那就不用算了
...
% --------------------------------
        ...
        [newmpc2,pipelineFailue] = topologicalUpdateFormpc(newGEresult1,newmpc1,gtd,Info_components(s,:),burstPressure_merged, ...
            rupturePressure_merged,delta_merged,wt,dl,M_ng,gamma_ng,R_air,T_gas,psi,kappa);
%-----------test------------------
% newmpc2=newmpc1;
% S7
% newmpc2 = newmpc1;% 不考虑管道的损坏
% --------------------------------
...
%         if pipelineFailue ~= 0
%             gasLoadCurtailment{s} = gasLoadCurtailment{s} / 24;
%         end
