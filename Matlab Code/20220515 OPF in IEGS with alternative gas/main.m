...
% !!这里gasComposition0(ii,:)的编号不应该是ii，而应该是第ii个gasload对应的节点编号！！
for ii = 1:nGasLoad
    QdGasCompositionCons = [
        QdGasCompositionCons;
        (gasComposition0(ii,:)) .* sum(Qd(ii,:),2) - sigma_Qd(ii,:) <= Qd(ii,:) <= (gasComposition0(ii,:)) .* sum(Qd(ii,:),2) + sigma_Qd(ii,:);
%         (gasComposition_ng(r)-5*xi)*sum(Qd,2) <= Qd(:,r) <= (gasComposition_ng(r)+5*xi)*sum(Qd,2); % 每个负荷消耗气体成分不能和原来的差太多
        ];
end
gasDemandCons = [
    QdGasCompositionCons;
...
% gpp
% !! （20220810）这里是不是错了，能量单位没有换算——噢不对，约掉了
...
electricityCons = [
    ...
    Pgmin <= Pg <= Pgmax;
    Va(13) == 1; % 除了slackbus外，其他相角都没约束。但是一些solver在处理inf的上下限的时候有问题
    ]:'electricityCons';
% wobbe index
