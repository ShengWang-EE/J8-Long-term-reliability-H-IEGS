...
proportion = [5,4,1,1,1,1];
newGsou = [];
newGasComposition = [];
newGcost = [];
for i = 1:size(Gsou,1)
    addGsou = repmat([Gsou(i,1),Gsou(i,2:4)/proportion(i)],[proportion(i),1]);
    addGasComposition = repmat(gasCompositionForGasSource(i,:),[proportion(i),1]);
    addGcost = repmat(Gcost(i,:),[proportion(i),1]);

    newGsou = [newGsou; addGsou];
    newGasComposition = [newGasComposition; addGasComposition];
    newGcost = [newGcost; addGcost];
end
mpc.Gsou=newGsou;
mpc.gasCompositionForGasSource=newGasComposition;
mpc.Gcost = newGcost;
% gas composition: Methane (CH4) Ethane (C2H6) Propane (C3H8) I-butane (C4H10) Hydrogen (H2) Nitrogen (N2) Carbon dioxide (CO2)
...
mpc.GEcon=GEcon;
%% ptg
% gas bus, electricity bus, electrolysis effciency, methanation efficiency,
% min cap, max cap (Mm3/day)
mpc.ptg=[
1   10  0.7 0.8 0   1 %ptg
10  17  0.7 0.8 0   1 %ptg
4   18  0.7 0.8 0   1  %ptg
];
% mpc.ptg=[
...
%% load level
rts.electricityLoad = [
    0.7     0.9     1.0 % load level
    0.7     0.9    1 % probability
    ];
rts.gasLoad = [
    0.8     0.9     1.0
    0.7     0.9    1
